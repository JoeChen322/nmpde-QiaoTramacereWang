...
Stokes::setup()
{
  ...
  // Initialize the DoF handler.
  {
    ...
    // Questo per non mischiare le componenti una volta distribuite le parti delle  
    // matrici del sistema e avere una separazione coerente con la teoria. Se lasciassimo 
    // la numerazione invariata, il sistema non sarebbe più a blocchi.
    ...
    // Ogni processore avrà own e relevant dofs per la pressione e per la velocità.
    // In questo modo dofs della pressione e della velocità non vengono mischiati tra i processori.
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  // Se i e j non corrispondono ad un dofs della velocità, FEValuesExtractors estrae 
                  // zero quando viene chiesta la velocità automaticamente. In modo analogo 
                  // lavora con la pressione.

                  // Questo è molto comodo dal punto di vista della programmazione perché usiamo un solo 
                  // ciclo. Esistono tecniche migliori che usano la rappresentazione per componenti.
