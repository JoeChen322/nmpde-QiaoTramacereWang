...
def compute_phase_drift_semi_T(df: pd.DataFrame) -> pd.Series:
    """
    Return drift vs semi-discrete phase at final time T.

    Your Wave currently stores phase_drift_T relative to omega_exact:
        phase_drift_T = phi(T) - (phi(0) + omega_exact*T)

    Convert to drift vs omega_semi:
    ...
    return df["phase_drift_T"].to_numpy() + (df["omega_exact"].to_numpy() - df["omega_semi"].to_numpy()) * df["T"].to_numpy()
...
def main():
    ...
    # Expect columns from your updated study:
    ...
    for (m, n), g in df.groupby(["m", "n"]):
        ...
        plt.plot(x, y, marker="o", label=f"(m,n)=({m},{n})")
    ...
    # Convert to "lag in cycles" (positive = behind)
    ...
    for (m, n), g in df2.groupby(["m", "n"]):
        ...
        # If T varies across rows, it will still plot correctly; but typically T is fixed.
        plt.plot(x, y, marker="o", label=f"(m,n)=({m},{n})")

    plt.xlabel(r"$\Delta t$")
    plt.ylabel(r"phase lag at $T$ (cycles, + = behind)")
    # Use a single T in title if it is constant; otherwise omit the value.
    T_unique = np.unique(df2["T"].to_numpy())
    if len(T_unique) == 1:
        plt.title(f"Accumulated phase lag vs Δt (θ=0.5, T={T_unique[0]:g})")
    else:
        plt.title("Accumulated phase lag vs Δt (θ=0.5)")
    plt.grid(True)
    plt.legend()
    ...
    # Optional: also save the enriched dataframe for debugging/post-processing
    # df2.to_csv(os.path.join(out_dir, "dispersion_summary_with_phase_lag.csv"), index=False)
